...
public:
    ...
    */
    ...
    int updateObjInter(int typeU, int typeV) {
        //shuffle the moves:        
        ...
        while (!isEndSearch)
        {
            isEndSearch = true;
            //isMoved = 0;                        
            for (int posU = 0; posU < ordNodeLs.size() && isMoved == 0; ++posU) {
                ...
                for (int posV = 0; posV < lstNeig->size() && isMoved == 0; ++posV) {
                    ...
                    if (routeU != routeV) {
                        ...
                        if (isMoved != 1) {
                            ...
                            vSuc = nodeV->suc;
                            vPred = nodeV->pred;
                            isMoved = interRouteGeneralInsert(typeU, typeV);
                            nodeV = tempNode;
                            vSuc = nodeV->suc;
                            vPred = nodeV->pred;
                        }
    ...
    int interRouteGeneralInsert(int typeU, int typeV) {
        int iBest = 0, jBest = 0;
        int moveMin;
        // 0 -> send nothing
        // 1 -> send U
        ...
        if (difDir == 2) {
            ...
            else
            {
                ...
                if (vPred->idxClient) {
                    ...
                    myseqs1.push_back(vSuc->seqi_n);
                    resGenInMoves[0][3] = seq->evaluation(myseqs1);
                }
                //1-1:
                if (nodeU->idxClient == 0)goto line1;
                myseqs1.clear();
        ...
        while (isImproved)
        {
            ...
            for (int i = 1; i <= 6; ++i) {                
                ...
                case 6:
                    ...
                    break;
                    /*case 7:
                        isImproved=(SwapTailLocalSearch())?true:isImproved;
                        break;*/
                default:
                    break;
                }
