...
public:
    ...
    Solution(Param* _pr) {
        ...
        m = pr->numVeh;
        // create giant tour
        for (int i = 1; i <= n + 1; ++i)giantT.pb(0);
        //create node
        //Node *nodes = new Node [n+2*m+1];
    ...
    double caculateETSP() {
        double res = pr->listCL[0].calEnergy(pr->listCL[giantT[1]], 0);
        int load = pr->listCL[giantT[1]].demand;
        for (int i = 2; i <= n; ++i) {
            res += pr->listCL[giantT[i - 1]].calEnergy(pr->listCL[giantT[i]], load);
            load += pr->listCL[giantT[i]].demand;
        }
        res += pr->listCL[giantT[n]].calEnergy(pr->listCL[0], load);
        return res;
    }
    ...
    double caculateE() {
        double res = 0.0;
        for (int i = 1; i <= m; ++i) {
            //cout << bestSol.solT[bestSol.setR[i]->depot->posInSol] << " ";
            //bestSol.setR[i]->showR();  
            setR[i]->updateRoute();
            res += setR[i]->calEInRan(0, setR[i]->length);
            cout << "testE: " << setR[i]->calEInRan(0, setR[i]->length) << endl << endl;;
            cout << "testQ: " << setR[i]->calQInRan(0, setR[i]->length) << endl << endl;;
        }
        return res;
    }

    double calDis() {
        double res = pr->costs[giantT[0]][giantT[1]] + pr->costs[giantT[n]][giantT[0]];
        for (int i = 2; i <= n; ++i) {
            res += pr->costs[giantT[i - 1]][giantT[i]];
        }
        return res;
    }
    ...
    void Split() {
        ...
        for (int i = 0; i <= n - 1; ++i) {
            ...
            for (int j = i + 1; j <= n; ++j) {
                ...
                if (j == i + 1) {
                    dis += pr->costs[0][idxJ];
                    energy += pr->listCL[0].calEnergy(pr->listCL[idxJ], load);
                }
                else {
                    dis += pr->costs[idxI][idxJ];
                    energy += pr->listCL[idxI].calEnergy(pr->listCL[idxJ], load);
                }                
                load += pr->listCL[idxJ].demand;                
                if (energy + pr->listCL[idxJ].calEnergy(pr->listCL[0], load) > pr->maxE)continue;
                if (load > pr->cap)break;                    
                for (int k = 0; k <= m - 1; ++k) {
                    if (F[k][i] != oo && F[k][i] + dis + pr->costs[idxJ][0] < F[k + 1][j]) {
                        F[k + 1][j] = F[k][i] + dis + pr->costs[idxJ][0];
                        pred[k + 1][j] = i;
                    }
                }
            }
    ...
    double ckEnerCWH(int u, int v, bool isUpdate = false) {
        Route* routeU = nodes[u]->rou;
        Route* routeV = nodes[v]->rou;
        int loadU = routeU->calQInRan(0, routeU->length);
        int loadV = routeV->calQInRan(0, routeV->length);
        if (loadU + loadV > pr->cap)return pr->maxE + 1.0;
        double EnergyU, EnergyV;
        double AofV;
        bool reverseU = false;
        bool reverseV = false;
        if (nodes[u]->suc != routeU->depot->pred) {
            reverseU = true;
            EnergyU = routeU->calEInRan(routeU->length, nodes[u]->posInRoute);
        }
        else {
            EnergyU = routeU->calEInRan(0, nodes[u]->posInRoute);
        }

        if (nodes[v]->pred != routeV->depot) {
            reverseV = true;
            EnergyV = routeV->calEInRan(nodes[v]->posInRoute, 0);
            AofV = routeV->calAInRan(nodes[v]->posInRoute, 0);
        }
        else {
            EnergyV = routeV->calEInRan(nodes[v]->posInRoute, routeV->length);
            AofV = routeV->calAInRan(nodes[v]->posInRoute, routeV->length);
        }
        
        //update:
        if (isUpdate) {            
            if (reverseU)routeU->reverse();
            if (reverseV)routeV->reverse();

            //depot:
            nodes[u]->suc = nodes[v];
            nodes[v]->pred = nodes[u];
            Node* endNodeV = routeV->depot->pred->pred;
            endNodeV->suc = routeU->depot->pred;
            routeU->depot->pred->pred = endNodeV;            
            routeU->updateRoute();            
            routeV->depot = nullptr;// DELETE ROUTE_V  
            return pr->maxE;
        }

        return EnergyU + EnergyV +
            loadU * (AofV + pr->listCL[u].calA(pr->listCL[v]))
            + pr->listCL[u].calEnergy(pr->listCL[v], 0);
    }
    //innit sol for using Clarke and Wright savings heuristic
    void initSol() {
        vector<Route*> setOfR;
